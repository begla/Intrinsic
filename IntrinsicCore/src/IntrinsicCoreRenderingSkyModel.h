...
#define TERRESTRIAL_SOLAR_RADIUS (glm::radians<double>(0.51) / 2.0)
  ...
  cookConfiguration(ArHosekSkyModelDataset p_Dataset,
                    ArHosekSkyModelConfiguration p_Config, double p_Turbidity,
                    double p_Albedo, double p_SolarElevation)
  {
    double* elevMatrix;
    ...
    for (uint32_t i = 0; i < 9; ++i)
    {
      ...
      p_Config[i] += (p_Albedo) * (1.0 - turbidityRem) *
                     (pow(1.0 - p_SolarElevation, 5.0) * elevMatrix[i] +
                      5.0 * pow(1.0 - p_SolarElevation, 4.0) *
                          p_SolarElevation * elevMatrix[i + 9] +
                      10.0 * pow(1.0 - p_SolarElevation, 3.0) *
                          pow(p_SolarElevation, 2.0) * elevMatrix[i + 18] +
                      10.0 * pow(1.0 - p_SolarElevation, 2.0) *
                          pow(p_SolarElevation, 3.0) * elevMatrix[i + 27] +
    ...
    res = (1.0 - p_Albedo) * (1.0 - turbidityRem) *
          (pow(1.0 - p_SolarElevation, 5.0) * elevMatrix[0] +
           5.0 * pow(1.0 - p_SolarElevation, 4.0) * p_SolarElevation *
               elevMatrix[1] +
           10.0 * pow(1.0 - p_SolarElevation, 3.0) *
               pow(p_SolarElevation, 2.0) * elevMatrix[2] +
           10.0 * pow(1.0 - p_SolarElevation, 2.0) *
               pow(p_SolarElevation, 3.0) * elevMatrix[3] +
  ...
  calculateSkyModelRadianceRGB(const ArHosekSkyModelState& p_SkyModelState,
                               const glm::vec3& p_Theta,
                               const glm::vec3& p_Gamma)
  {
    glm::vec3 configuration[9];
    for (uint32_t i = 0; i < 9; ++i)
      configuration[i] = glm::vec3(p_SkyModelState.configs[0][i],
                                   p_SkyModelState.configs[1][i],
   ...
   */
  _INTR_INLINE static Rendering::IBL::SH9
          ...
          const glm::vec3& p_LightDir, uint32_t p_SampleCount)
  {
    ...
    for (uint32_t i = 0u; i < p_SampleCount; ++i)
    {
      ...
      result += Rendering::IBL::project(randomRaySample, sample);
    }
...
private:
  /**
   * RGB radiance dataset.
   */
  static double* datasetsRGBRad[3];

  /**
   * RGB dataset.
   */
  static double* datasetsRGB[3];
};
}
}
}